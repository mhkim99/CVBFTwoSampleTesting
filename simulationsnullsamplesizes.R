...
p1 = ggplot(dfBF, aes(x = datasetsize)) + geom_point(aes(y = AvgCVBF)) + geom_hline(yintercept = -log(20), color = "blue") + labs(x = "n", y = "log(CVBF)") + theme_minimal()
# ggsave("logCVBFnullvsn.pdf", plot = p1, device = "pdf")

p1 = ggplot(dfBF, aes(x = datasetsize)) + geom_point(aes(y = PTBF)) + geom_hline(yintercept = -log(20), color = "blue") + labs(x = "n", y = "log(BF)") + theme_minimal()
# ggsave("logPTBFnullvsn.pdf", plot = p1, device = "pdf")

p2 = ggplot(dfBF) +
  geom_point(aes(x = datasetsize, y = AvgCVBF), color = "black", size = 2) +
  geom_point(aes(x = datasetsize + 10, y = PTBF), color = "red", size = 2) +
  geom_hline(yintercept = -log(20), color = "blue") +
  labs(x = "n", y = "log(BF)") +
  theme_minimal() +
  theme(
...
ggsave("logBFsnullvsn.pdf", plot = p2, device = "pdf", width=10, height=6.15)
